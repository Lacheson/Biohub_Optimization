...
## Objective Function
EXPORTING_COST = 100 
total_cost = model.sum(f.ecocost * s_b_link[b, f] * Distance((b.x_coord,b.y_coord), (f.x_coord,f.y_coord)) for b in refinies for f in subdivs) + model.sum(f.envcost * s_b_link[b, f] * Distance((b.x_coord,b.y_coord), (f.x_coord,f.y_coord))  for b in refinies for f in subdivs) + EXPORTING_COST
...
for b_loc in refinies:
    if biorefinery_vars[b_loc].solution_value == 1:
        print('location changed', "location: ", b_loc.x_coord,b_loc.y_coord,  "demand:", b_loc.demand)
        DEMAND += b_loc.demand 
