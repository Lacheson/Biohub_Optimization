...
road_data = np.loadtxt('road_rail_data_real.txt', skiprows=1, usecols=0)
rail_data = np.loadtxt('road_rail_data_real.txt', skiprows=1, usecols=1)
...
class SubDivision(object):
    ...
    def __init__(self, x_coord, y_coord, farmdens, popdens, ghg, biodiv, soil, ecocost, envcost, road, rail):       
            self.x_coord = x_coord
            self.y_coord = y_coord
...
for i in range(0,37):
     subdivs.append(SubDivision(x[i], y[i], farm_density[i], pop_density[i], ghg_data[i], biodiv_data[i], soil_data[i], 0,0,road_data[i], rail_data[i]))


for i in range(0,37):
     refinies.append(Biorefinery(x[i], y[i]))
