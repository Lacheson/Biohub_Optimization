...
class Biorefinery(object):
        ...
        def __init__(self,  x_coord, y_coord, demand, capacity, control_indicator):         
...
for i in range(0,37): #creating a list of possible biorefineries
     refinies.append(Biorefinery(x[i], y[i], biomass_data_quantitydemand[i], biomass_data_capacity[i], 'none'))
...
for b in refinies:
     if b.capacity==CAP:
          print(b.x_coord, b.y_coord, b.capacity)
...
for b_loc in refinies:
    if biorefinery_vars[b_loc].solution_value == 1:
        print('location changed', "location: ", b_loc.x_coord,b_loc.y_coord, "Control:", b_loc.control_indicator, "cap:", b_loc.capacity)
