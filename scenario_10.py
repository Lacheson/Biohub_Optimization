...
biomass_data_typedemand = np.loadtxt('fake_biomass_data.txt', skiprows=1, usecols=0)
biomass_data_typesupply = np.loadtxt('fake_biomass_data.txt', skiprows=1, usecols=1)
...
biomass_data_capacity = np.loadtxt('fake_capacity_data.txt', skiprows=1)
...
class Biorefinery(object):
        ...
        def __init__(self,  x_coord, y_coord, biomasstype, demand,capacity, control_indicator):         
            self.x_coord = x_coord
            self.y_coord = y_coord
            self.biomasstype = biomasstype
            self.demand = demand
...
class SubDivision(object):
    ...
    def __init__(self, x_coord, y_coord, farmdens, popdens, ghg, biodiv, soil, ecocost, envcost, road, rail, biomasstype, supply):       
            self.x_coord = x_coord
            self.y_coord = y_coord
...
for i in range(0,37):
     subdivs.append(SubDivision(x[i], y[i], farm_density[i], pop_density[i], ghg_data[i], biodiv_data[i], soil_data[i], 0,0,road_data[i], rail_data[i], biomass_data_typesupply[i], biomass_data_quantitysupply[i]))
...
for i in range(0,37):
     refinies.append(Biorefinery(x[i], y[i], biomass_data_typedemand[i], biomass_data_quantitydemand[i], biomass_data_capacity[i], 'none'))
...
MAXBIODIV = 40
MAXSOIL = 40
CAP = max(biomass_data_capacity)  #calculating the maximums for capacity, supply, road, and rail
SUPPLY = max(biomass_data_quantitysupply)
MAXROAD = max(road_data)
MAXRAIL = max(rail_data)
...
for b in refinies:
    for f in subdivs:
        ...
        if b.biomasstype != f.biomasstype:
             model.add_constraint(s_b_link[b, f] == 0)

        if b.demand > f.supply:
             model.add_constraint(s_b_link[b,f]==0) #SCENARIO DEFINING CONSTRAINT

#This constraint says if the biorefinery has the largest capacity, and the farm has the largest supply, road, and rail access then the biorefinery gets to have this farm
        if b.capacity!=CAP and f.road != MAXROAD and f.rail!=MAXRAIL and f.supply!=SUPPLY:
             model.add_constraint(s_b_link[b, f] == 0)
             b.control_indicator = 'No Control'

        if b.capacity==CAP and f.road == MAXROAD and f.rail==MAXRAIL and f.supply==SUPPLY:
             #model.add_constraint(s_b_link[b,f]==1)
             b.control_indicator = 'Control'
...
print(DEMAND, '=', sum(biomass_data_quantitysupply))
